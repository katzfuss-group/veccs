...
vector<int> maxmin(py::array_t<double> &locations)
{
  // input
  py::buffer_info info = locations.request();

  unsigned int N = info.shape[0];
  unsigned int dim = info.shape[1];
  auto locations_raw_ptr = static_cast<double*>(info.ptr);
  ...
  for (int i = 0; i < (int)N; i++)
  {
    for(int j = 0; j < dim; j++)
    {
      coords[dim * i + j] = locations_raw_ptr[dim * i + j];
...
PYBIND11_MODULE(maxmin_cpp, m) {
  m.def("maxmin_cpp", &maxmin, "MinMax ordering");
}
